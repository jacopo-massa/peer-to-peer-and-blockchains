...
public class DTNHost implements Comparable<DTNHost> {
	...
	private HTEStatus hteStatus;
	private SirStatus sirStatus;
	private HashMap<HTEStatus, Double> hteProbabilities;
	...
	/** useful namespaces */
	private static final String HTE_NS = "HTE";
	...
	public DTNHost(List<MessageListener> msgLs,
		...
		this.sirStatus = SirStatus.SUSCEPTIBLE;
		this.hteStatus = HTEStatus.HALTING;
		this.hteProbabilities = new HashMap<>();
		...
		// setup HTE Model probabilities
		Settings s = new Settings(HTE_NS);
		EnumSet.allOf(HTEStatus.class).forEach(st -> {
			try {
				hteProbabilities.putIfAbsent(st, s.getDouble(st.toString()));
			} catch (SettingsError se) {
				System.err.println("Failed to find '" + HTE_NS + "." + st + " probability");
	...
	public void infectNode() {
		this.sirStatus = SirStatus.INFECTIOUS;
	}
	...
	public boolean isInfected() {
		return this.sirStatus.equals(SirStatus.INFECTIOUS);
	}
	...
	public boolean isSusceptible() {
		return this.sirStatus.equals(SirStatus.SUSCEPTIBLE);
	}
	...
	public void move(double timeIncrement) {
		...
		//this.hteStatus = HTEStatus.TRAVELLING;
	...
	private Coord exploreCoord() {
		...
		double y = random.nextInt(50) + this.location.getY();
		if(y > this.movement.getMaxY())
			y = this.movement.getMaxY();
	...
	private boolean isTravelling() {
		return this.hteStatus.equals(HTEStatus.TRAVELLING);
	}

	private boolean isHalted() {
		return this.hteStatus.equals(HTEStatus.HALTING);
	}

	private boolean isExploring() {
		return this.hteStatus.equals(HTEStatus.EXPLORING);
	}
