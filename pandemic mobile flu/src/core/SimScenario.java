...
public class SimScenario implements Serializable {
	...
	/** namespace for OS */
	private static final String OS_NS = "OperatingSystem";
	/** namespace for SIR Model */
	private static final String SIR_NS = "SIR";
	...
	public static HashMap<SirStatus, Double> sirProbabilities;

	public static final String CONTACT_FILENAME = new SimpleDateFormat("'contacts' yyyy-MM-dd HH.mm.ss'.txt'").format(new Date());
	...
	protected SimScenario() {
		...
		try {
			boolean dirCreated = new File(Paths.get("logs").toString()).mkdir();
			String CONTACT_PATH = "logs/" + CONTACT_FILENAME;
			bw = new BufferedWriter(new FileWriter(CONTACT_PATH,true));
		}catch (IOException e) {e.printStackTrace();}

		initSirProbabilities();
		createHosts();
	...
	private void initSirProbabilities() {
		sirProbabilities = new HashMap<>();
		Settings s = new Settings(SIR_NS);
		EnumSet.allOf(SirStatus.class).forEach(st -> {
			try {
				sirProbabilities.putIfAbsent(st, s.getDouble(st.toString()));
			} catch (SettingsError se) {
				System.err.println("Failed to find '" + SIR_NS + "." + st + " probability");
	...
	protected void createHosts() {
		...
		// setup Operating Systems' probabilities
		Settings settings = new Settings(OS_NS);
		ArrayList<Double> probabilities = new ArrayList<>();
		EnumSet.allOf(OperatingSystem.class).forEach(os -> {
			try {
				probabilities.add(settings.getDouble(os.toString()));
			} catch (SettingsError se) {
				System.err.println("Failed to find " + os + " probability");
				System.err.println("Caught at " + se.getStackTrace()[0]);
				System.exit(-1);
			}
		});
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				// choose randomly an os, following the defined probabilities
				Random r = new Random();
				double randomNumber = r.nextDouble();
				double probSum = 0;
				int idx = 0;
				while(randomNumber > probSum) {
					probSum += probabilities.get(idx);
					idx++;
				}

				host.setOS(OperatingSystem.values()[idx-1]);
				hosts.add(host);
			}
		}

		// infect randomly one host per OS
		Collections.shuffle(this.hosts);
		EnumSet.allOf(OperatingSystem.class).forEach(os -> {
			for(DTNHost h: hosts) {
				if(h.getOS().equals(os)) {
					h.infectNode();
					break;
				}
			}
		...
		// write on the log file, the total number of hosts
		try {
			bw.write(hosts.size() + "\n");
		}catch(IOException e) {
			e.printStackTrace();
