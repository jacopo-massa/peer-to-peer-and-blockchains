...
import movement.RandomHotspot;
import movement.RandomWaypoint;
...
public class PlayField extends JPanel {
	...
	public void paint(Graphics g) {
		...
		// draw hotsposts
		if(RandomHotspot.hotspots != null) {
			double range = 50;
			g2.setColor(Color.DARK_GRAY);
			for(Coord h: RandomHotspot.hotspots) {
				Ellipse2D ellipse = new Ellipse2D.Double(scale(h.getX()-range),
						scale(h.getY()-range), scale(range * 2),
						scale(range * 2));
				g2.draw(ellipse);
			}
		}
	...
	public Coord getGraphicsPosition(Coord loc) {
		Coord c = loc.clone();
		c.setLocation(scale(c.getX()) + PLAYFIELD_OFFSET,
				scale(c.getY()) + PLAYFIELD_OFFSET);
		return c;
	}
